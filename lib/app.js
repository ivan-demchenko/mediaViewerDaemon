...
function processFiles(filesToProcess) {
  return () => {
    ...
    var processCurrentFileFn = R.flip(processFile)(fileToProcess);

    checkFileFullyProcessed(fileToProcess)
    .then(R.difference(['thumb', 'preview']))
    .then((missingItems) => {
      return missingItems.length ?
              Q.all(R.map(processCurrentFileFn, missingItems)) :
              'all is fine';
    })
    .then((processingResult) => {
      let processRestOfFiles = processFiles(R.tail(filesToProcess));
      if (processingResult === 'all is fine') {
        processRestOfFiles();
      } else {
        setTimeout(processRestOfFiles, userConfig.processingPause);
      }
    })
