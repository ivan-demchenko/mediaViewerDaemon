...
const cachePath = R.curryN(3, path.join)(userConfig.cacheRootDir);
const cachedCopyExists = (type, src) => Q.nfcall(fs.access, cachePath(type, src)).then(R.always(true)).catch(R.always(false));
const getSizeStr = (type) => `${config.photos[type].w}x${config.photos[type].h}`;
const isPhoto = R.compose(R.lt(0), R.length, R.match(/\.(jpeg|jpg)$/));
const nonDotFile = R.compose(R.not, R.equals(0), R.length, R.match(/^\./));
